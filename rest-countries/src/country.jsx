...
function Country() {
  ...
  return (
    <div className={`${theme === 'light' ? 'bg-lightBackground' : 'bg-darkBackground'} min-h-screen p-10`}>
        <div className={`${theme === 'light' ? 'bg-light' : 'bg-dark'} flex flex-wrap p-2 bg-black w-fit justify-evenly content-center rounded-lg cursor-pointer shadow-lg`} onClick={handleBackClick}>
                <img src={theme === 'light' ? arrowB : arrow} alt='arrow' className=''></img>
                <span className={theme === 'light' ? 'text-lightText' : 'text-light'}>Back</span>
        </div>
        <div className='mt-10'>
        {filteredCountries.map(country => (
            ...
            <div key={country.name} className='flex'>
               <div className='w-2/4 h-fit'> <img src={country.flags.svg} alt={country.name} className='w-2/3'/></div>
                <div className={`${theme === 'light' ? 'text-lightText' : 'text-light'} w-2/4`}>
                ...
                <div className='mt-5 flex'>
                    ...
                    {country.borders && country.borders.length > 0 ? (
                        country.borders.map((border, index) => (
                            filteredCountries.map(country =>(
                                country.alpha3Code === border ? (<Link to={`/${country.name}`}><li key={index} className={`${theme === 'light' ? 'bg-light' : 'bg-dark'} mr-5 mb-2 shadow-lg p-2`}>{country.name}</li></Link>) : null
                            ))
                        ))
                    ) : (
